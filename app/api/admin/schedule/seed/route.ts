...
const SEED_ENTRIES = [
  ...
  { subject:'CS 316',     title:'Network Security Adv.',            section:'BSCS 4-A',  days:['T','F'],     start_time:'15:00:00', end_time:'16:30:00', room:'CB 318',  building:'CB',  instructor_name:'Prof. Lim' },
  ...
  { subject:'ACCT 401',   title:'Financial Reporting',              section:'BSBA 4-A',  days:['T','F'],     start_time:'09:00:00', end_time:'10:30:00', room:'CBS 405', building:'CBS', instructor_name:'Prof. Campos' },
  ...
  { subject:'TOUR 441',   title:'Cultural Heritage Mgmt',           section:'BSTM 4-D',  days:['M','TH'],    start_time:'12:00:00', end_time:'13:30:00', room:'CBE 410', building:'CBE', instructor_name:'Prof. Santiago' },
...
export async function POST() {
  ...
  // Insert clean seed data in batches of 50
  const BATCH = 50
  for (let i = 0; i < SEED_ENTRIES.length; i += BATCH) {
    const batch = SEED_ENTRIES.slice(i, i + BATCH)
    const { error: insertError } = await admin.from('schedule_entries').insert(batch)
    if (insertError) return NextResponse.json({ error: insertError.message }, { status: 500 })
  }

  return NextResponse.json({ success: true, inserted: SEED_ENTRIES.length })
}
